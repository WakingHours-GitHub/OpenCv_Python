...
# range：要统计特征的取值范围。在上例中，range = [0, 255], 一般都是uint8所以是0~255

# 直方图的意义：
# 直方图是图像中像素强度(亮度)分布的图形表达方式。
# 它统计了每一个强度值所具有的像素个数。
# 不同的图像的直方图可能是相同的。
...
# histSize:BINS 的数目。也应该用中括号括起来，例如：[256]。-> 分成多少块 　　
...
# img_gray = cv.imread("../../img/space_boat.jpg", cv.IMREAD_GRAYSCALE)
plt.imshow(img, cmap=plt.cm.gray)
plt.show()
space_boat_hsh = cv.calcHist([img], [0], None, [256], [0, 256])
# 参数分别是: img, 通道, 掩膜, bins, 像素值范围
...
plt.plot(space_boat_hsh) # 可以直接使用plot进行画图
# 因为hist图形使用的是折现图
plt.show()
